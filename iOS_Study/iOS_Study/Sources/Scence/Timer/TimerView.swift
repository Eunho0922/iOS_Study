...
class CircularProgressView: UIView {
    ...
    fileprivate func createProgressView(){
        ...
        let circularPath = UIBezierPath(arcCenter: center, radius: frame.width / 2, startAngle: CGFloat(3.5 * .pi), endAngle: CGFloat(1.5 * .pi), clockwise: false)
    ...
    func setProgress(duration: TimeInterval = 3, to newProgress: Float) -> Void{
        ...
        progressLayer.strokeStart = CGFloat(newProgress)
        progressLayer.strokeEnd = 1.0
        progressLayer.add(animation, forKey: "animationProgress")
        
        let label = UILabel()
        label.text = String(timeToFill)
    ...
    func stopProgress() {
        if let presentationLayer = progressLayer.presentation() {
            let currentStrokeStart = presentationLayer.strokeStart
            progressLayer.strokeStart = currentStrokeStart
        }
        progressLayer.removeAnimation(forKey: "animationProgress")
    }
