...
class TimerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if count.truncatingRemainder(dividingBy: 3600) == 0 {
            initTimerLabel.text = "\(Int(time[0]))시간"
        } else if count / 3600 > 1  {
            initTimerLabel.text = "\(Int(time[0]))시간  \(Int(time[1]))분"
        } else if count.truncatingRemainder(dividingBy: 60) == 0 {
            initTimerLabel.text = "\(Int(time[1]))분"
        } else if count / 60 > 1{
